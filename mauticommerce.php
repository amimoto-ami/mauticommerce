...
if (!mautic_is_activate_woocommerce()) {
    ...
    add_action('admin_notices', [$Mauticommerce_Err, 'admin_notices']);
...
class Mauticommerce
{
    ...
    public static function text_domain()
    {
        ...
        if (!$text_domain) {
            $data = get_file_data(Mauticommerce_ROOT, ['text_domain' => 'Text Domain']);
    ...
    public function init()
    {
        ...
        add_action('admin_init', [$admin, 'settings_init']);
        $order = Mauticommerce_Order::get_instance();
        add_action('woocommerce_checkout_update_order_meta', [$order, 'subscribe_to_mautic']);
    }
}
